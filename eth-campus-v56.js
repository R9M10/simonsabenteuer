(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V56__) return;
  ...
  // Bahnhofstrasse locker is installed by simon-ui-v37 at x=996.
  // Put Polybahn next to it, but leave enough space that the two hit zones do
  // not overlap.
  const DEFAULT_BAHNHOF_LOCKER_X = 996;
  const POLYBAHN_OFFSET_FROM_LOCKER = -118;
  ...
  function canUsePolybahn(scene) {
    return Boolean(
      ...
      !scene.__polybahnTravelActiveV56 &&
      polybahnStoryUnlocked(scene)
    );
  }

  function getPolybahnBahnhofX(scene) {
    const lockerX =
      Number(scene?.__sv37LockerObjects?.zone?.x) ||
      DEFAULT_BAHNHOF_LOCKER_X;

    return lockerX + POLYBAHN_OFFSET_FROM_LOCKER;
  }
  ...
  function refreshCampusEntryAvailability(scene) {
    const entry = scene?.__ethCampusEntryV56;
    if (!entry) return;
    ...
    if (entry.zone?.input) {
      ...
      entry.zone.input.cursor = unlocked ? "pointer" : "default";
    }

    if (unlocked) {
      entry.facade?.setAlpha?.(1);
      entry.sign?.setAlpha?.(1);

      if (!entry.marker?.active) {
        entry.marker = scene.createPulsingInteractionMarker?.(
          entry.x,
          GROUND_TOP - 30,
          176
        ) || null;
      ...
      // Visible world detail, but completely silent and non-interactive.
      entry.facade?.setAlpha?.(0.72);
      entry.sign?.setAlpha?.(0.72);

      if (entry.marker?.active) {
        scene.tweens?.killTweensOf?.(entry.marker);
  ...
  function destroyBahnhofPolybahn(scene) {
    ...
    entry.facade?.destroy?.(true);
    entry.sign?.destroy?.();
    entry.zone?.destroy?.();
    entry.marker?.destroy?.();

    scene.__ethCampusEntryV56 = null;
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV56) return;

    const x = getPolybahnBahnhofX(scene);
    const bottom = GROUND_TOP;

    const facade = scene.add.container(0, 0).setDepth(7);
    const g = scene.add.graphics();

    // Small Polybahn kiosk/portal next to the locker. It is intentionally
    // compact so it reads as an interaction point, not a second explorable
    // district beside the Bahnhofstrasse.
    g.fillStyle(0x776f67, 1);
    g.fillRoundedRect(x - 29, 208, 58, bottom - 208, 5);

    g.fillStyle(0xb63132, 1);
    g.fillRoundedRect(x - 34, 179, 68, 34, 5);
    g.lineStyle(2, 0xf1dfc5, 1);
    g.strokeRoundedRect(x - 34, 179, 68, 34, 5);

    g.fillStyle(0x29454e, 1);
    g.fillRoundedRect(x - 20, 226, 40, bottom - 226, 4);

    // Door split.
    g.lineStyle(2, 0xb9c6c6, 0.9);
    g.lineBetween(x, 230, x, bottom - 3);
    ...
    // Marker is intentionally created only after the Orell-WEITSICHT step.
    let marker = null;

    zone.on("pointerdown", (pointer) => {
      stopEvent(pointer?.event);

      if (!canUsePolybahn(scene)) return;
      startPolybahnFromBahnhof(scene);
    });

    scene.__ethCampusEntryV56 = {
      x,
      facade,
      sign,
      zone,
  ...
  function prepareBahnhofForPolybahn(station) {
    station.__polybahnTravelActiveV56 = true;
    station.__polybahnTravelActiveV55 = true;
    station.__polybahnReturnYV56 = station.player?.y || 235;
    station.__polybahnReturnYV55 = station.__polybahnReturnYV56;
  ...
  function restoreBahnhofAfterPolybahn(station) {
    if (!station) return;

    station.__polybahnTravelActiveV56 = false;
    station.__polybahnTravelActiveV55 = false;
    station.__ethCampusEnteringV56 = false;

    station.cameras?.main?.resetFX?.();
    station.player?.setVisible?.(true);

    if (station.player?.body) {
      station.player.body.enable = true;
    }

    station.player?.setPosition?.(
      getPolybahnBahnhofX(station) - 70,
      Number(
        station.__polybahnReturnYV56 ??
        station.__polybahnReturnYV55
      ) || 235
    );
    ...
    station.blockWorldInteractions?.(380);
    station.ensureTicketMachineInteractive?.();
    station.ensureLockerInteractive?.();
    station.ensureTramBoardingInteractive?.();
    station.syncStreetStoreHitboxes?.();
    station.updateCoinHUD?.();
    station.updateHpBar?.();

    if (!station.hotbarDOM?.isConnected) {
      station.hotbarDOM = null;
      station.refreshHotbar?.();
    }

    station.updateHotbarActionUI?.();
    station.updateSprintIndicator?.(true);
    station.refreshAbilityTouchControl?.();
    station.refreshWeaponTouchControl?.();
  }
  ...
  function startPolybahnFromBahnhof(station) {
    ...
    if (
      ...
      console.error("ETH v56: Polybahn-Szenen fehlen.");
      return false;
    }

    prepareBahnhofForPolybahn(station);

    station.__ethCampusEnteringV56 = true;
    ...
    station.time?.delayedCall?.(180, () => {
      ...
      try {
        ...
        game.scene.start(TRANSIT_KEY, {
          overworld: station,
          direction: "up"
        });
      } catch (error) {
        console.error("ETH v56: Polybahn-Start fehlgeschlagen:", error);
        game.scene.resume?.("BahnhofquaiScene");
        restoreBahnhofAfterPolybahn(station);
      }
    });
  ...
  class PolybahnTransitScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.time.delayedCall(5200, () => {
        if (
          ...
          console.warn("ETH v56: Polybahn-Transit per Failsafe beendet.");
          this.finishTransit();
        }
      });
    ...
    finishTransit() {
      ...
      this.time.delayedCall(155, () => {
        try {
          if (up) {
            // Starts Terrace and atomically shuts down the current transit.
          ...
          // IMPORTANT ordering on return:
          // 1) wake Bahnhof
          // 2) fully restore its player/UI/state
          // 3) only then stop this transit scene
          try {
            game?.scene?.resume?.("BahnhofquaiScene");
          } catch {}

          restoreBahnhofAfterPolybahn(world);

          this.scene.stop();
        } catch (error) {
          console.error("ETH v56: Polybahn-Ende fehlgeschlagen:", error);
    ...
    rideDown() {
      ...
      this.time.delayedCall(170, () => {
        try {
          ...
          console.error("ETH v56: Rückfahrt konnte nicht starten:", error);
    ...
    enterETH() {
      ...
      this.time.delayedCall(190, () => {
        try {
          ...
          console.error("ETH v56: ETH-Innenraumstart fehlgeschlagen:", error);
...
// Einstein dialogue / quiz helpers
  ...
  function createDialogueOverlay(scene, onAdvance) {
    ...
    overlay.dataset.simonUi = "eth-dialogue-v56";
  ...
  function openQuizModal(scene, question) {
    ...
    const overlay = document.createElement("div");
    overlay.dataset.simonUi = "eth-quiz-v56";
    ...
    leaveETH() {
      ...
      this.time.delayedCall(200, () => {
        try {
          ...
          console.error("ETH v56: Rückkehr auf Polyterrasse fehlgeschlagen:", error);
        } finally {
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV56(...args) {
  ...
  function ensureRegistered(game) {
    ...
    scenes.forEach(([key, SceneClass]) => {
      ...
      try {
        ...
        console.error(
          `ETH v56: ${key} konnte nicht registriert werden:`,
          error
        );
      }
    });
  ...
  function recoverPolybahn(game) {
    ...
    const transit = getScene(game, TRANSIT_KEY);
    const terrace = getScene(game, TERRACE_KEY);
    const ethScene = getScene(game, ETH_KEY);

    const transitActive = Boolean(transit?.sys?.isActive?.());
    const terraceActive = Boolean(terrace?.sys?.isActive?.());
    const ethActive = Boolean(ethScene?.sys?.isActive?.());

    const travelActive = Boolean(
      station?.__polybahnTravelActiveV56 ||
      station?.__polybahnTravelActiveV55
    );

    // ETH interior owns the foreground; Terrace must be paused beneath it.
    if (
      ethActive &&
    ...
    // During actual travel/destination play, Bahnhof remains paused.
    if (
      station?.sys?.isActive?.() &&
      travelActive &&
      (transitActive || terraceActive || ethActive) &&
      !station.sys?.isPaused?.()
    ) {
      try {
        game.scene.pause("BahnhofquaiScene");
      } catch {}
    ...
    // Nothing from the Polybahn stack remains -> Bahnhof must be live.
    if (
      station &&
      travelActive &&
      !transitActive &&
    ...
    // Additional hard recovery: a Bahnhof scene that is still paused even
    // though travel state is already cleared must never remain frozen.
    if (
      station?.sys?.isPaused?.() &&
      !travelActive &&
      !transitActive &&
  ...
  function startEthDeveloper(game) {
    ...
    const attempt = () => {
      ...
      if (
        ...
        const x = getPolybahnBahnhofX(station);
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV56(options = {}) {
        ...
        hitboxes: {
          bahnhofPolybahnX:
            getScene(game, "BahnhofquaiScene")
              ? getPolybahnBahnhofX(getScene(game, "BahnhofquaiScene"))
              : null,
  ...
  console.info(
    "ETH Campus v56: Polybahn direkt am Locker, robuste Transit-Szene, Polyterrasse outdoor, Einstein mittig."
  );
})();
