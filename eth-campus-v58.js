(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V58__) return;
  ...
  function refreshCampusEntryAvailability(scene) {
    const entry = scene?.__ethCampusEntryV58;
    if (!entry) return;
  ...
  function destroyBahnhofPolybahn(scene) {
    ...
    entry.sign?.destroy?.();
    entry.centralLabel?.destroy?.();
    entry.zone?.destroy?.();
    entry.marker?.destroy?.();

    scene.__ethCampusEntryV58 = null;
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV58) return;
    ...
    // The real lower station is integrated into the Central street frontage,
    // rather than standing as a free kiosk. Build a compact historic facade
    // between the ticket/locker area and Der Inder.
    g.fillStyle(0xb9ad9c, 1);
    g.fillRect(
      x - 100,
      94,
      200,
    ...
    // Upper windows make it read as a city building facade.
    [-55, 5].forEach((dx) => {
      g.fillStyle(0x435b64, 1);
      g.fillRect(
        x + dx,
        119,
        48,
    ...
    // Ground-floor darker stone strip.
    g.fillStyle(0x80766d, 1);
    g.fillRect(
      x - 100,
      184,
      200,
    ...
    // Red Polybahn brand panel.
    g.fillStyle(0xb72f31, 1);
    g.fillRoundedRect(
      x - 83,
      182,
      166,
    ...
    // Small red funicular icon.
    g.fillStyle(0xca3a3c, 1);
    g.fillRoundedRect(
      x - 22,
      230,
      44,
    ...
    g.fillStyle(0xe4eeee, 1);
    g.fillRect(x - 15, 236, 11, 9);
    g.fillRect(x + 4, 236, 11, 9);
    ...
    // A tiny Central context label integrates the entrance into the street
    // without turning this into another walkable Central level.
    const centralLabel = scene.add.text(
      x,
      106,
      "CENTRAL",
      {
  ...
  function startPolybahnFromBahnhof(station) {
    ...
    if (
      ...
      console.error("ETH v58: Polybahn-Szenen fehlen.");
      return false;
    }
    ...
    station.time?.delayedCall?.(180, () => {
      ...
      try {
        ...
        console.error(
          "ETH v58: Polybahn-Start fehlgeschlagen:",
          error
        );
    ...
    create() {
      ...
      this.time.delayedCall(5200, () => {
        if (
          ...
          console.warn("ETH v58: Polybahn-Transit per Failsafe beendet.");
          this.finishTransit();
        }
      });
    ...
    finishTransit() {
      ...
      this.time.delayedCall(155, () => {
        try {
          ...
          console.error(
            "ETH v58: Polybahn-Ende fehlgeschlagen:",
            error
          );
    ...
    rideDown() {
      ...
      this.time.delayedCall(170, () => {
        try {
          ...
          console.error("ETH v58: Rückfahrt konnte nicht starten:", error);
    ...
    enterETH() {
      ...
      this.time.delayedCall(190, () => {
        try {
          ...
          console.error("ETH v58: ETH-Innenraumstart fehlgeschlagen:", error);
...
// Einstein dialogue / quiz helpers
    ...
    leaveETH() {
      ...
      this.time.delayedCall(200, () => {
        try {
          ...
          console.error("ETH v58: Rückkehr auf Polyterrasse fehlgeschlagen:", error);
        } finally {
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV58(...args) {
  ...
  function ensureRegistered(game) {
    ...
    scenes.forEach(([key, SceneClass]) => {
      ...
      try {
        ...
        console.error(
          `ETH v58: ${key} konnte nicht registriert werden:`,
          error
        );
      }
    });
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV58(options = {}) {
  ...
  console.info(
    "ETH Campus v58: Polybahn links von der Tram, robuste Transit-Szene, Polyterrasse outdoor, Einstein mittig."
  );
})();
