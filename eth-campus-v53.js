(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V53__) return;
  window.__SIMON_ETH_CAMPUS_V53__ = true;
  ...
  const CENTRAL_KEY = "PolybahnCentralScene";
  const RIDE_KEY = "PolybahnRideScene";
  const TERRACE_KEY = "PolyterrasseScene";
  const ETH_KEY = "ETHInteriorScene";

  const CENTRAL_WIDTH = 1320;
  ...
  // Keep the former public state key alive for compatibility.
  ...
  function showLocalNotice(scene, title, detail = "", duration = 2400) {
    ...
    node.dataset.simonUi = "eth-notice-v53";
  ...
  //
  // Keep this intentionally far left of the Bahnhofquai tram shelter. The
  // station hotspot begins around x=610; the campus route starts around x=285.
  // ---------------------------------------------------------------------------
  ...
  function canEnterFromBahnhof(scene) {
    return Boolean(
      ...
      !scene.fightActive &&
      !scene.playerDying &&
      !scene.__ethCampusEnteringV53
    );
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV53) return;
    ...
    // A small side-street gateway rather than another button beside the tram.
    g.fillStyle(0x8e887e, 1);
    g.fillRoundedRect(x - 58, 168, 116, bottom - 168, 5);

    g.fillStyle(0x4b4e4e, 1);
    g.fillRoundedRect(x - 39, 198, 78, bottom - 198, 4);

    // Steps disappear uphill into Central.
    ...
    g.fillStyle(0xb53634, 1);
    g.fillRoundedRect(x - 59, 151, 118, 29, 4);
    g.lineStyle(2, 0xf1e4cf, 1);
    g.strokeRoundedRect(x - 59, 151, 118, 29, 4);
    ...
    scene.events?.once?.("shutdown", () => {
      scene.__ethCampusEntryV53 = null;
    });
  }

  function enterCentral(station) {
    ...
    if (!game.scene.keys?.[CENTRAL_KEY]) {
      console.error("ETH v53: Central-Szene wurde nicht registriert.");
      return false;
    }

    station.__ethCampusEnteringV53 = true;
    station.__ethCampusReturnYV53 = station.player.y;
    ...
    window.setTimeout(() => {
      try {
        ...
        game.scene.start(CENTRAL_KEY, {
          overworld: station,
          spawn: "city"
        });

        station.__ethCampusEnteringV53 = false;
      } catch (error) {
        console.error("ETH v53: Central-Übergang fehlgeschlagen:", error);
        station.__ethCampusEnteringV53 = false;
  ...
  // ---------------------------------------------------------------------------
  // Central — proper horizontal walking area and lower Polybahn station
  // ---------------------------------------------------------------------------

  let BaseScene = getBaseSceneClass();

  class PolybahnCentralScene extends (BaseScene || Phaser.Scene) {
    constructor() {
      super(CENTRAL_KEY);
    ...
    create() {
      ...
      this.physics.world.setBounds(0, 0, CENTRAL_WIDTH, GAME_HEIGHT);
      this.cameras.main.setBounds(0, 0, CENTRAL_WIDTH, GAME_HEIGHT);
      this.cameras.main.setBackgroundColor("#9bb6c1");
      ...
      createEthPlayer(
        this,
        this.spawn === "polybahn" ? 1000 : 135,
        235
      );

      buildLocalControls(this, { showRideHint: true });
      this.createCityExit();
      this.createLowerStation();
    ...
    createCentralArchitecture() {
      ...
      // Sky and distant Zürich.
      g.fillStyle(0x9ebac5, 1);
      g.fillRect(0, 0, CENTRAL_WIDTH, 145);

      g.fillStyle(0x718f86, 0.75);
      g.fillTriangle(0, 205, 245, 128, 515, 205);
      g.fillTriangle(410, 205, 690, 142, 980, 205);

      // Limmat behind the street.
      g.fillStyle(0x668f9e, 1);
      g.fillRect(0, 228, 690, 55);
      g.lineStyle(2, 0xa5c5ca, 0.56);
      for (let x = 15; x < 675; x += 62) {
        g.lineBetween(x, 246, x + 38, 246);
        g.lineBetween(x + 21, 263, x + 53, 263);
      }

      // Old Central / Niederdorf-like façade rhythm.
      const buildings = [
        { x: 0,   w: 112, top: 116, c: 0xb6a48d },
        { x: 116, w: 121, top: 132, c: 0xc4b49b },
        { x: 241, w: 104, top: 107, c: 0xa99883 },
        { x: 349, w: 126, top: 127, c: 0xc0a993 },
        { x: 479, w: 111, top: 113, c: 0xa99d8c },
        { x: 594, w: 116, top: 136, c: 0xc7b7a0 }
      ];

      buildings.forEach(({ x, w, top, c }, idx) => {
        g.fillStyle(c, 1);
        g.fillRect(x, top, w, 119);

        g.fillStyle(0x4e5659, 1);
        for (let wy = top + 24; wy < top + 94; wy += 37) {
          for (let wx = x + 15; wx < x + w - 13; wx += 31) {
            g.fillRect(wx, wy, 14, 22);
          }
        }

        g.fillStyle(0x4f4b47, 1);
        g.fillTriangle(
          x - 3,
          top,
          x + w / 2,
      ...
      // Street: rails in background, curb, then proper pavement under Simon.
      g.fillStyle(0x5d6060, 1);
      g.fillRect(0, 272, CENTRAL_WIDTH, 31);

      g.lineStyle(3, 0x3f4344, 1);
      g.lineBetween(0, 282, 890, 282);
      g.lineBetween(0, 295, 890, 295);

      g.fillStyle(0xb9b4aa, 1);
      g.fillRect(0, 303, CENTRAL_WIDTH, 35);

      g.fillStyle(0xd0cbc1, 1);
      g.fillRect(0, 303, CENTRAL_WIDTH, 7);

      g.lineStyle(1, 0x8d8982, 0.75);
      for (let x = 0; x < CENTRAL_WIDTH; x += 48) {
        g.lineBetween(x, 310, x, 338);
      }
      g.lineBetween(0, 324, CENTRAL_WIDTH, 324);

      // Lower Polybahn station as an actual building with entrance.
      const left = 990;
      const top = 118;
      const width = 285;

      g.fillStyle(0x81786f, 1);
      g.fillRoundedRect(left, top, width, GROUND_TOP - top, 6);

      g.fillStyle(0xb63132, 1);
      g.fillRect(left, top + 35, width, 34);

      g.lineStyle(3, 0xe5d9c3, 1);
      g.strokeRoundedRect(left, top, width, GROUND_TOP - top, 6);

      // Arched glass station entrance.
      g.fillStyle(0x344b54, 1);
      g.fillCircle(1125, 232, 58);
      g.fillRect(1067, 232, 116, 106);

      g.fillStyle(0x20343b, 1);
      g.fillCircle(1125, 236, 45);
      g.fillRect(1080, 236, 90, 102);

      g.lineStyle(4, 0xdad0bc, 1);
      g.strokeCircle(1125, 232, 58);

      // Poster case and little ticket window.
      g.fillStyle(0x6c5f50, 1);
      g.fillRect(1008, 208, 58, 76);
      g.fillStyle(0xe6ddc8, 1);
      g.fillRect(1015, 216, 44, 58);
    ...
    createCityExit() {
      const zone = this.add.zone(62, 272, 112, 128)
        .setDepth(230)
        .setInteractive({ useHandCursor: true });
      ...
      this.__cityExitZone = zone;
    }

    createLowerStation() {
      const zone = this.add.zone(1125, 265, 135, 150)
        .setDepth(235)
        .setInteractive({ useHandCursor: true });
      ...
      this.__polyLowerZone = zone;
      this.__polyLowerMarker = marker;
    }

    boardPolybahn() {
      if (isSceneBusy(this)) return;
      ...
      this.time.delayedCall(200, () => {
        ...
        game?.scene?.stop?.(CENTRAL_KEY);
        game?.scene?.start?.(RIDE_KEY, {
      ...
      this.time.delayedCall(200, () => {
        ...
        if (world) {
          ...
          world.player?.setPosition?.(
            330,
            Number(world.__ethCampusReturnYV53) || 235
          );
  ...
  // ---------------------------------------------------------------------------
  // Real Polybahn ride — no fake walkable diagonal floor.
  // ---------------------------------------------------------------------------

  class PolybahnRideScene extends Phaser.Scene {
    constructor() {
      super(RIDE_KEY);
      this.overworld = null;
      this.direction = "up";
    ...
    create() {
      this.cameras.main.setBackgroundColor("#9eb8c1");
      this.cameras.main.resetFX();
      this.cameras.main.fadeIn(180, 0, 0, 0);

      this.createRideVisuals();
    ...
    createRideVisuals() {
      // Distant city parallax layer.
      const city = this.add.container(0, 0).setDepth(-20);
      const cg = this.add.graphics();

      cg.fillStyle(0x9eb8c1, 1);
      cg.fillRect(0, 0, GAME_WIDTH, GAME_HEIGHT);

      cg.fillStyle(0x6c8e85, 0.75);
      cg.fillTriangle(-40, 250, 170, 144, 390, 250);
      cg.fillTriangle(280, 250, 560, 165, 870, 250);

      cg.fillStyle(0x668e9e, 0.82);
      cg.fillRect(-20, 265, 480, 65);

      // Houses along the slope.
      const colors = [0xb9a891, 0xc6b69f, 0x9d8f80, 0xb5a18d];
      for (let i = 0; i < 9; i += 1) {
        const x = -35 + i * 103;
        const y = 185 - i * 11;
        const h = 105 + (i % 3) * 14;
        ...
        cg.fillRect(x, y, 91, h);

        cg.fillStyle(0x4c575b, 1);
        for (let wy = y + 23; wy < y + h - 18; wy += 35) {
          cg.fillRect(x + 16, wy, 14, 21);
          cg.fillRect(x + 53, wy, 14, 21);
        }
      }

      city.add(cg);
      this.__rideCity = city;

      // Retaining wall and foliage layer, moving slightly faster.
      const slope = this.add.container(0, 0).setDepth(-10);
      const sg = this.add.graphics();

      sg.fillStyle(0x77766f, 1);
      sg.fillTriangle(80, 390, 820, 42, 820, 390);

      sg.lineStyle(2, 0x97968f, 0.68);
      for (let i = 0; i < 18; i += 1) {
        const x = 155 + i * 47;
        const y = 355 - i * 21;
        sg.lineBetween(x - 24, y, x + 24, y);
      }

      sg.fillStyle(0x3d6e51, 1);
      for (let i = 0; i < 12; i += 1) {
        const x = 80 + i * 69;
        const y = 315 - i * 20;
        sg.fillCircle(x, y, 19);
        sg.fillCircle(x + 17, y - 7, 15);
      }

      slope.add(sg);
      this.__rideSlope = slope;

      // Fixed rails provide a clear spatial reference for the cabin.
      const rails = this.add.graphics().setDepth(2);

      rails.lineStyle(9, 0x4d4c49, 1);
      rails.lineBetween(152, 334, 662, 88);

      rails.lineStyle(3, 0xd1c7b4, 1);
      rails.lineBetween(143, 337, 653, 91);
      rails.lineBetween(161, 331, 671, 85);

      for (let t = 0.02; t < 0.98; t += 0.06) {
        const x = Phaser.Math.Linear(152, 662, t);
        const y = Phaser.Math.Linear(334, 88, t);

        rails.lineStyle(3, 0x41413e, 1);
        rails.lineBetween(x - 15, y + 5, x + 15, y - 5);
      }

      // Actual red carriage. Simon is visibly inside it.
      const cabin = this.add.container(0, 0).setDepth(30);

      const body = this.add.graphics();
      body.fillStyle(0xb62f31, 1);
      body.fillRoundedRect(-56, -43, 112, 83, 8);

      body.lineStyle(4, 0xf0dfc4, 1);
      body.strokeRoundedRect(-56, -43, 112, 83, 8);

      body.fillStyle(0x29454e, 1);
      body.fillRect(-43, -31, 31, 32);
      body.fillRect(12, -31, 31, 32);

      body.fillStyle(0xe7dac1, 1);
      body.fillRect(-5, -37, 10, 70);

      body.fillStyle(0x3f3e3d, 1);
      body.fillCircle(-34, 43, 10);
      body.fillCircle(34, 43, 10);
      ...
      this.add.text(
        ...
        28,
        up
          ? "POLYBAHN · CENTRAL → POLYTERRASSE"
          : "POLYBAHN · POLYTERRASSE → CENTRAL",
        {
          fontFamily: '"Press Start 2P", monospace',
          fontSize: "5.5px",
          color: "#fff0cf",
    ...
    startRide() {
      ...
      this.__rideCabin.setPosition(from.x, from.y);

      // Parallax: the city visibly sinks while the carriage climbs.
      this.tweens.add({
        targets: this.__rideCity,
        y: up ? 86 : -86,
        x: up ? -24 : 24,
        duration: 3250,
        ease: "Sine.easeInOut"
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          ...
          this.time.delayedCall(210, () => {
            ...
            game?.scene?.stop?.(RIDE_KEY);

            if (up) {
              game?.scene?.start?.(TERRACE_KEY, {
                overworld: this.overworld,
                spawn: "polybahn"
              });
            } else {
  ...
  // ---------------------------------------------------------------------------
  // Polyterrasse — separate, fully walkable top area with proper stone paving.
  // ---------------------------------------------------------------------------
  ...
  class PolyterrasseScene extends (BaseScene || Phaser.Scene) {
    ...
    create() {
      ...
      this.cameras.main.setDeadzone(255, 92);
      this.cameras.main.roundPixels = true;
    ...
    createTerraceArchitecture() {
      ...
      bg.fillStyle(0xa9a49b, 1);
      bg.fillRect(0, 275, TERRACE_WIDTH, 63);

      // Proper large-format stone paving under the player.
      bg.fillStyle(0xc3beb5, 1);
      bg.fillRect(0, 286, TERRACE_WIDTH, 52);

      bg.fillStyle(0xd1ccc2, 1);
      bg.fillRect(0, 286, TERRACE_WIDTH, 7);

      bg.lineStyle(1, 0x98948d, 0.75);
      ...
      // Upper Polybahn station far left.
      ...
      // UZH as direct neighboring university, set behind the terrace.
      const uzh = this.add.graphics().setDepth(-7);
      const uLeft = 860;
      const uTop = 90;

      uzh.fillStyle(0xb9b3a6, 1);
      uzh.fillRect(uLeft, uTop, 300, 196);

      uzh.fillStyle(0x979188, 1);
      uzh.fillRect(uLeft - 10, uTop - 10, 320, 15);

      for (let y of [uTop + 43, uTop + 105]) {
        for (let x = uLeft + 28; x < uLeft + 280; x += 47) {
          uzh.fillStyle(0x506a73, 1);
          uzh.fillRect(x, y, 22, 34);
          uzh.lineStyle(2, 0xe3daca, 1);
          uzh.strokeRect(x, y, 22, 34);
        }
      }
      ...
      // ETH main building — broad, regular Semper-inspired façade.
      ...
      // Benches / bikes / students stop the terrace feeling empty.
      [455, 700].forEach((x) => {
    ...
    createUpperStation() {
      const zone = this.add.zone(164, 255, 190, 140)
        .setDepth(230)
        .setInteractive({ useHandCursor: true });
    ...
    createEthEntrance() {
      const zone = this.add.zone(1458, 260, 132, 145)
        .setDepth(235)
        .setInteractive({ useHandCursor: true });
    ...
    enterETH() {
      ...
      this.time.delayedCall(210, () => {
        try {
          ...
          console.error("ETH v53: Innenraumstart fehlgeschlagen:", error);
    ...
    resumeFromETH() {
      ...
      // ETH writes coins / HP / inventory into the authoritative Bahnhof scene.
      // Refresh this paused terrace copy before it can later sync back.
...
// Einstein dialogue / quiz helpers
  ...
  function createDialogueOverlay(scene, onAdvance) {
    ...
    overlay.dataset.simonUi = "eth-dialogue-v53";
  ...
  function openQuizModal(scene, question) {
    ...
    const overlay = document.createElement("div");
    overlay.dataset.simonUi = "eth-quiz-v53";
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV53(...args) {
  ...
  function install(game) {
    ...
    const scenes = [
      [CENTRAL_KEY, PolybahnCentralScene],
      [RIDE_KEY, PolybahnRideScene],
    ...
    scenes.forEach(([key, SceneClass]) => {
      ...
      try {
        ...
        console.error(`ETH v53: ${key} konnte nicht registriert werden:`, error);
      }
    });
  ...
  function addDeveloperButton() {
    ...
    button.innerHTML =
      '8. POLYBAHN / ETH' +
      '<small>Ab Central die neue Polybahn-Fahrt, Polyterrasse und Einstein testen.</small>';
  ...
  function startEthDeveloper(game) {
    ...
    const attempt = () => {
      ...
      if (
        ...
        game.scene.keys?.[CENTRAL_KEY] &&
        game.scene.keys?.[RIDE_KEY] &&
        ...
        station.setUILocked?.(true);
        station.__ethCampusReturnYV53 = station.player?.y || 235;
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV53(options = {}) {
  ...
  // Compatibility for existing developer-console bookmarks.
  window.SimonETHV51 = publicApi;

  console.info(
    "ETH Campus v53 geladen: Central, echte Polybahn-Fahrt, Polyterrasse, ETH und Einstein."
  );
})();
