(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V51__) return;
  window.__SIMON_ETH_CAMPUS_V51__ = true;
  ...
  const ROUTE_KEY = "UniversityRouteScene";
  const ETH_KEY = "ETHInteriorScene";

  const ROUTE_WIDTH = 1640;
  ...
  function showLocalNotice(scene, title, detail = "", duration = 2400) {
    ...
    node.dataset.simonUi = "eth-notice-v51";
  ...
  // ---------------------------------------------------------------------------
  // Bahnhofstrasse / HB entrance
  // ---------------------------------------------------------------------------

  function canEnterFromBahnhof(scene) {
    return Boolean(
      scene?.sys?.isActive?.() &&
      scene.player?.active &&
      !scene.uiLocked &&
      ...
      !scene.fightActive &&
      !scene.playerDying &&
      !scene.__ethCampusEnteringV51
    );
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV51) return;

    const x = 545;
    const bottom = GROUND_TOP;

    const facade = scene.add.container(0, 0).setDepth(6);
    const g = scene.add.graphics();

    // Narrow stone stair/portal immediately left of the Bahnhofquai shelter.
    g.fillStyle(0x858078, 1);
    g.fillRoundedRect(x - 48, 176, 96, bottom - 176, 5);

    g.fillStyle(0x494b4b, 1);
    g.fillRoundedRect(x - 34, 201, 68, bottom - 201, 4);

    // Ascending stair hint.
    g.fillStyle(0xb7b0a3, 1);
    for (let i = 0; i < 5; i += 1) {
      g.fillRect(
        x - 29 + i * 7,
        bottom - 17 - i * 8,
    ...
    // Characteristic red Polybahn sign.
    g.fillStyle(0xb83432, 1);
    g.fillRoundedRect(x - 48, 160, 96, 27, 4);
    g.lineStyle(2, 0xf4e3cf, 1);
    g.strokeRoundedRect(x - 48, 160, 96, 27, 4);
    ...
    zone.on("pointerdown", (pointer) => {
      ...
      if (!canEnterFromBahnhof(scene)) return;

      enterUniversityRoute(scene);
    });

    scene.__ethCampusEntryV51 = {
      facade,
      sign,
      sub,
      zone,
    ...
    scene.events?.once?.("shutdown", () => {
      scene.__ethCampusEntryV51 = null;
    });
  }

  function enterUniversityRoute(station) {
    if (!canEnterFromBahnhof(station)) return false;

    const game = getGame() || station.game || null;
    if (!game?.scene) return false;

    install(game);

    if (!game.scene.keys?.[ROUTE_KEY]) {
      console.error("ETH v51: UniversityRouteScene wurde nicht registriert.");
      return false;
    }

    station.__ethCampusEnteringV51 = true;
    station.__ethCampusReturnYV51 = station.player.y;

    station.player.setVelocity?.(0, 0);
    station.blockWorldInteractions?.(520);
    station.setUILocked?.(true);

    const transition = () => {
      try {
        station.player?.setVisible?.(false);
        if (station.player?.body) station.player.body.enable = false;

        game.scene.pause("BahnhofquaiScene");
        game.scene.start(ROUTE_KEY, {
          overworld: station
        });

        station.__ethCampusEnteringV51 = false;
      } catch (error) {
        console.error("ETH v51: Übergang fehlgeschlagen:", error);
        station.__ethCampusEnteringV51 = false;
        station.cameras?.main?.resetFX?.();
        station.player?.setVisible?.(true);
        if (station.player?.body) station.player.body.enable = true;
        station.setUILocked?.(false);
      }
    };

    station.cameras?.main?.fadeOut?.(190, 0, 0, 0);
    window.setTimeout(transition, 210);
    return true;
  }

  // ---------------------------------------------------------------------------
  // Route scene — Central / Polybahn / Polyterrasse
  // ---------------------------------------------------------------------------

  let BaseScene = getBaseSceneClass();

  class UniversityRouteScene extends (BaseScene || Phaser.Scene) {
    constructor() {
      super(ROUTE_KEY);
    ...
    create() {
      ...
      this.physics.world.setBounds(0, 0, ROUTE_WIDTH, GAME_HEIGHT);
      this.cameras.main.setBounds(0, 0, ROUTE_WIDTH, GAME_HEIGHT);
      this.cameras.main.setBackgroundColor("#98b4c4");
      this.cameras.main.resetFX();
      this.cameras.main.fadeIn(220, 0, 0, 0);

      this.createRouteArchitecture();
    ...
    createRouteArchitecture() {
      const bg = this.add.graphics().setDepth(-30);

      // Sky gradient-ish bands.
      bg.fillStyle(0x9dbbca, 1);
      bg.fillRect(0, 0, ROUTE_WIDTH, 145);
      bg.fillStyle(0xb8c9cf, 1);
      bg.fillRect(0, 145, ROUTE_WIDTH, 75);

      // Distant hills / Uetliberg silhouette.
      bg.fillStyle(0x708f84, 0.82);
      bg.fillTriangle(0, 200, 250, 107, 525, 200);
      bg.fillTriangle(270, 200, 565, 124, 860, 200);
      bg.fillTriangle(900, 195, 1210, 120, 1580, 195);

      // Limmat / lower Central glimpse.
      bg.fillStyle(0x638fa0, 1);
      bg.fillRect(0, 251, 520, 45);

      bg.lineStyle(2, 0x9bc0c8, 0.65);
      for (let x = 20; x < 500; x += 55) {
        bg.lineBetween(x, 266, x + 35, 266);
        bg.lineBetween(x + 18, 280, x + 50, 280);
      }

      // Old Zürich façades around Central.
      const facadeColors = [
        0xb8a58c,
        0xc6b69b,
        0x9e8975,
      ...
      for (let i = 0; i < 7; i += 1) {
        const x = 0 + i * 88;
        const top = 118 + (i % 3) * 13;
        const h = 185 - (i % 2) * 17;

        bg.fillStyle(facadeColors[i % facadeColors.length], 1);
        bg.fillRect(x, top, 82, h);

        bg.fillStyle(0x514a45, 1);
        bg.fillTriangle(x - 4, top, x + 41, top - 37, x + 86, top);

        bg.fillStyle(0x48606a, 1);
        for (let wy = top + 28; wy < top + h - 34; wy += 42) {
          for (let wx = x + 14; wx < x + 70; wx += 30) {
            bg.fillRect(wx, wy, 14, 22);
          }
        }
      }

      // CENTRAL street level.
      bg.fillStyle(0x817b73, 1);
      bg.fillRect(0, 296, 720, 42);
      bg.fillStyle(0xaaa39a, 1);
      bg.fillRect(0, 296, 720, 8);
      ...
      // Steep Polybahn alignment.
      const rail = this.add.graphics().setDepth(-5);

      rail.lineStyle(8, 0x615b58, 1);
      rail.lineBetween(582, 299, 930, 137);
      rail.lineStyle(3, 0xd5c6a6, 1);
      rail.lineBetween(574, 299, 922, 137);
      rail.lineBetween(590, 299, 938, 137);

      for (let t = 0; t <= 1; t += 0.065) {
        const x = Phaser.Math.Linear(582, 930, t);
        const y = Phaser.Math.Linear(299, 137, t);

        rail.lineStyle(3, 0x514e4b, 1);
        rail.lineBetween(x - 13, y + 4, x + 13, y - 4);
      }

      // Lower Polybahn station canopy.
      rail.fillStyle(0x463c3a, 1);
      rail.fillRoundedRect(520, 213, 142, 82, 5);
      rail.fillStyle(0xb73834, 1);
      rail.fillRect(520, 206, 142, 22);
      rail.lineStyle(3, 0xddd1b8, 1);
      rail.strokeRoundedRect(520, 213, 142, 82, 5);
      ...
      // Upper panorama: old town, Grossmünster twin towers and lake.
      const skyline = this.add.graphics().setDepth(-12);

      skyline.fillStyle(0x6d7373, 0.76);

      for (let x = 860; x < 1640; x += 44) {
        const h = 32 + ((x / 44) % 4) * 10;
        skyline.fillRect(x, 235 - h, 38, h);
        skyline.fillTriangle(x - 3, 235 - h, x + 19, 218 - h, x + 41, 235 - h);
      }

      // Grossmünster-ish twin towers in panorama.
      [1040, 1077].forEach((x) => {
        skyline.fillRect(x, 143, 25, 92);
        skyline.fillTriangle(x - 3, 143, x + 12, 112, x + 28, 143);
        skyline.fillRect(x + 9, 124, 6, 13);
      });

      // Lake opening.
      skyline.fillStyle(0x6e9dab, 0.8);
      skyline.fillRect(900, 235, 310, 45);

      // Polyterrasse stone carpet / balustrade.
      const terrace = this.add.graphics().setDepth(-3);
      terrace.fillStyle(0xa6a19a, 1);
      terrace.fillRect(900, 280, 360, 58);
      terrace.fillStyle(0xc1bbb0, 1);
      terrace.fillRect(900, 270, 360, 14);

      terrace.fillStyle(0x77736f, 1);
      terrace.fillRect(900, 251, 360, 9);
      for (let x = 910; x < 1250; x += 22) {
        terrace.fillRect(x, 258, 6, 20);
      }
      ...
      // UZH visible as the neighboring building.
      const uzh = this.add.graphics().setDepth(-4);
      uzh.fillStyle(0xb7b1a3, 1);
      uzh.fillRect(1030, 96, 190, 174);
      uzh.fillStyle(0x8f8b82, 1);
      uzh.fillRect(1019, 88, 212, 15);

      for (let x = 1050; x <= 1195; x += 37) {
        uzh.fillStyle(0x536d75, 1);
        uzh.fillRect(x, 126, 18, 31);
        uzh.fillRect(x, 178, 18, 31);
      }
      ...
      // ETH Semper-inspired main façade.
      const eth = this.add.graphics().setDepth(-2);
      const left = 1260;
      const width = 355;
      const top = 70;

      eth.fillStyle(0xc8bda9, 1);
      eth.fillRect(left, top, width, GROUND_TOP - top);

      eth.fillStyle(0xaaa08f, 1);
      eth.fillRect(left - 10, top + 28, width + 20, 15);
      eth.fillRect(left - 12, top + 179, width + 24, 14);

      // Central projecting pavilion.
      eth.fillStyle(0xd7ccb8, 1);
      eth.fillRect(left + 112, top - 22, 132, GROUND_TOP - top + 22);
      ...
      // Repeated tall arched windows.
      for (const y of [top + 62, top + 128]) {
        for (let x = left + 31; x < left + width - 20; x += 49) {
          eth.fillStyle(0x49636c, 1);
          eth.fillCircle(x, y, 10);
          eth.fillRect(x - 10, y, 20, 28);
          eth.lineStyle(2, 0xe7deca, 1);
      ...
      // Grand entrance.
      eth.fillStyle(0x3a3633, 1);
      eth.fillCircle(1438, GROUND_TOP - 76, 38);
      eth.fillRect(1400, GROUND_TOP - 76, 76, 76);
      eth.fillStyle(0x27353a, 1);
      eth.fillRect(1410, GROUND_TOP - 69, 56, 69);
      eth.lineStyle(4, 0xe0d3b9, 1);
      eth.strokeCircle(1438, GROUND_TOP - 76, 38);
      ...
      // Red lower cabin.
      this.__polyCabin = this.add.container(590, 264).setDepth(25);
      const cabin = this.add.graphics();
      cabin.fillStyle(0xb72f31, 1);
      cabin.fillRoundedRect(-39, -31, 78, 60, 6);
      cabin.lineStyle(3, 0xf0dfc3, 1);
      cabin.strokeRoundedRect(-39, -31, 78, 60, 6);
      cabin.fillStyle(0x29444c, 1);
      cabin.fillRect(-29, -21, 22, 24);
      cabin.fillRect(7, -21, 22, 24);
      cabin.fillStyle(0xe2d4b6, 1);
      cabin.fillRect(-4, -26, 8, 51);
      this.__polyCabin.add(cabin);
    }

    createPolybahnInteractions() {
      this.__polyLowerZone = this.add.zone(
        590,
        270,
        150,
        130
      ...
      this.__polyUpperZone = this.add.zone(
        945,
        254,
        150,
        140
      ...
      this.__polyUpperZone.setVisible(false);
      this.__polyUpperZone.input.enabled = false;

      const lowerMarker = this.createPulsingInteractionMarker?.(
        590,
        287,
        176
      );

      const upperMarker = this.createPulsingInteractionMarker?.(
        945,
        287,
        176
      );

      upperMarker?.setVisible?.(false);

      this.__polyLowerMarker = lowerMarker;
      this.__polyUpperMarker = upperMarker;

      this.__polyLowerZone.on("pointerdown", (pointer) => {
        stopEvent(pointer?.event);
        this.ridePolybahn(true);
      });

      this.__polyUpperZone.on("pointerdown", (pointer) => {
        stopEvent(pointer?.event);
        this.ridePolybahn(false);
      });
    }

    ridePolybahn(up) {
      if (isSceneBusy(this) || this.__ethAtTop === up) return;
      ...
      this.__polyCabin.setPosition(from.x, from.y);
      this.player.setPosition(from.x, from.y - 5);
      this.player.play?.("simon-idle", true);

      this.cameras.main.stopFollow();

      this.tweens.add({
        targets: [this.__polyCabin, this.player],
        x: to.x,
        y: to.y,
        duration: 1650,
        ease: "Sine.easeInOut",
        onUpdate: () => {
          this.cameras.main.centerOn(
            this.player.x,
            Phaser.Math.Clamp(this.player.y, 150, 260)
          );
        },
        onComplete: () => {
          this.__ethAtTop = up;
          ...
          if (up) {
            this.player.setPosition(980, 235);
            this.__polyCabin.setPosition(930, 138);
            this.cameras.main.scrollX = 820;
          } else {
            this.player.setPosition(500, 235);
            this.__polyCabin.setPosition(590, 264);
          ...
          this.__polyLowerZone.setVisible(!up);
          this.__polyLowerZone.input.enabled = !up;
          this.__polyLowerMarker?.setVisible?.(!up);

          this.__polyUpperZone.setVisible(up);
          this.__polyUpperZone.input.enabled = up;
          this.__polyUpperMarker?.setVisible?.(up);

          showLocalNotice(
            this,
            up ? "POLYTERRASSE" : "CENTRAL",
            up ? "ETH ZÜRICH" : "POLYBAHN",
            1600
          );
        }
      });
    ...
    createEthEntrance() {
      ...
      zone.on("pointerdown", (pointer) => {
        ...
        if (!this.__ethAtTop || isSceneBusy(this)) return;
    ...
    enterETH() {
      ...
      this.time.delayedCall(210, () => {
        try {
          ...
          game.scene.pause(ROUTE_KEY);
          game.scene.start(ETH_KEY, {
            overworld: this.overworld,
            routeScene: this
          });
        } catch (error) {
          console.error("ETH v51: Innenraumstart fehlgeschlagen:", error);
          this.player.setVisible(true);
          if (this.player.body) this.player.body.enable = true;
          this.__ethTransitionActive = false;
          setLocalUiVisible(this, true);
          this.cameras.main.resetFX();
        }
      });
    }
    ...
    resumeFromETH() {
      ...
      this.player?.setPosition?.(1365, 235);
      this.player?.setVelocity?.(0, 0);
      this.cameras.main.resetFX();
      this.cameras.main.startFollow(this.player, true, 0.10, 0.10);
      setLocalUiVisible(this, true);
      refreshLocalHotbar(this);
    }
    ...
    leaveRoute() {
      ...
      this.time.delayedCall(200, () => {
        game?.scene?.stop?.(ROUTE_KEY);
        game?.scene?.resume?.("BahnhofquaiScene");

        if (world) {
          world.cameras?.main?.resetFX?.();

          world.player?.setVisible?.(true);

          if (world.player?.body) {
            world.player.body.enable = true;
          }

          world.player?.setPosition?.(
            555,
            Number(world.__ethCampusReturnYV51) || 235
          );

          world.player?.setVelocity?.(0, 0);
          world.uiLocked = false;
          world.setUILocked?.(false);
          world.refreshUILock?.();
          world.setControlsVisible?.(true);
          world.blockWorldInteractions?.(420);
          world.ensureTicketMachineInteractive?.();
          world.ensureLockerInteractive?.();
          world.ensureTramBoardingInteractive?.();
          world.updateCoinHUD?.();
          world.updateHpBar?.();
        }
      });
    ...
    update(time) {
      ...
      // Walking into the ETH portal also works without tapping.
      if (
        this.__ethAtTop &&
        this.player?.x > 1404 &&
        !isSceneBusy(this)
  ...
  function createDialogueOverlay(scene, onAdvance) {
    ...
    overlay.dataset.simonUi = "eth-dialogue-v51";
  ...
  function openQuizModal(scene, question) {
    ...
    const overlay = document.createElement("div");
    overlay.dataset.simonUi = "eth-quiz-v51";
  ...
  function resolveAnswer(scene, question, selectedIndex) {
    ...
    runDialogue(
      scene,
      [
        { speaker: "einstein", text: "Das war mutig." },
        { speaker: "simon", text: "Aber richtig?" },
  ...
  class ETHInteriorScene extends (BaseScene || Phaser.Scene) {
    ...
    init(data = {}) {
      this.__simonInteriorScene = true;
      this.routeScene = data.routeScene || null;
      this.exiting = false;
      this.__ethTransitionActive = false;
      this.__ethDialogueActive = false;
      this.__ethItemBusy = false;
    ...
    createEinstein() {
      ...
      const statue = this.add.container(x, bottom).setDepth(20);
      ...
      this.__einsteinObstacle = obstacle;
    }
  ...
  // ---------------------------------------------------------------------------
  // Registration / patching / developer mode
  // ---------------------------------------------------------------------------
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV51(...args) {
  ...
  function install(game) {
    ...
    if (!game.scene.keys?.[ROUTE_KEY]) {
      try {
        game.scene.add(ROUTE_KEY, UniversityRouteScene, false);
      } catch (error) {
        console.error("ETH v51: Route-Szene konnte nicht registriert werden:", error);
      }
    }

    if (!game.scene.keys?.[ETH_KEY]) {
      try {
        game.scene.add(ETH_KEY, ETHInteriorScene, false);
      } catch (error) {
        console.error("ETH v51: Innenraum konnte nicht registriert werden:", error);
      }
    }
  ...
  function addDeveloperButton() {
    ...
    button.innerHTML =
      '8. ETH / EINSTEIN' +
      '<small>Direkt Polyterrasse, ETH-Innenraum und Physikquiz testen.</small>';
  ...
  function startEthDeveloper(game) {
    ...
    const attempt = () => {
      ...
      if (
        ...
        game.scene.keys?.[ROUTE_KEY] &&
        game.scene.keys?.[ETH_KEY]
      ) {
        ...
        if (station.player?.body) {
          station.player.body.enable = false;
        }
        ...
        window.setTimeout(() => {
          const route = getScene(game, ROUTE_KEY);

          if (!route?.sys?.isActive?.()) return;

          route.__ethAtTop = true;
          route.player?.setPosition?.(1360, 235);
          route.__polyCabin?.setPosition?.(930, 138);

          if (route.__polyLowerZone) {
            route.__polyLowerZone.setVisible(false);
            route.__polyLowerZone.input.enabled = false;
          }

          route.__polyLowerMarker?.setVisible?.(false);

          if (route.__polyUpperZone) {
            route.__polyUpperZone.setVisible(true);
            route.__polyUpperZone.input.enabled = true;
          }

          route.__polyUpperMarker?.setVisible?.(true);
          route.cameras.main.scrollX = 820;
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV51(options = {}) {
  ...
  window.SimonETHV51 = Object.freeze({
    VERSION,
    state,
    QUESTIONS,
    ...
    enter() {
      ...
      return enterUniversityRoute(station);
    },
  ...
  console.info(
    "ETH Campus v51 geladen: Polybahn, Polyterrasse, ETH-Hauptgebäude und Einstein-Quiz (100 Fragen)."
  );
})();