(() => {
  ...
  if (window.__SIMON_UI_V35__) return;
  window.__SIMON_UI_V35__ = true;
  ...
  const LOCKER_STATE = window.__SIMON_LOCKER_V35__ || {
  ...
  window.__SIMON_LOCKER_V35__ = LOCKER_STATE;
  ...
  function ensureStyles() {
    if (document.getElementById("simon-ui-v35-style")) return;

    const style = document.createElement("style");
    style.id = "simon-ui-v35-style";
    style.textContent = `
      #phaser-game [data-simon-ui="inder-v35-room"] {
        position: absolute;
        inset: 0;
        z-index: 99980;
        overflow: hidden;
      ...
      #phaser-game .sv35-inder-room-bg {
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
      ...
      #phaser-game .sv35-inder-seller {
        ...
        height: 170px;
        z-index: 3;
        background-image: url("inder-sprites-v35.png?v=35");
      ...
      #phaser-game .sv35-locker-section {
        min-width: 0;
      ...
      @media (max-width: 620px) {
        ...
        #phaser-game .sv35-locker-row button {
          grid-column: 1 / -1;
        }
      }
    `;
  ...
  function patchPurchases(scene) {
    ...
    if (
      ...
      !scene.purchaseStoreItem.__sv35AutoHotbar
    ) {
      const original = scene.purchaseStoreItem.bind(scene);

      const wrapped = function purchaseStoreItemV35(itemKey, ...args) {
      ...
      wrapped.__sv35AutoHotbar = true;
      scene.purchaseStoreItem = wrapped;
    }

    if (
      typeof scene.purchaseBook === "function" &&
      !scene.purchaseBook.__sv35AutoHotbar
    ) {
      const original = scene.purchaseBook.bind(scene);

      const wrapped = function purchaseBookV35(bookKey, ...args) {
  ...
  function patchQuoteBanner(scene) {
    if (
      ...
      scene.showRandomBookQuote.__sv35QuoteSize
    ) {
      return;
    }
    ...
    const wrapped = function showRandomBookQuoteV35(bookKey, ...args) {
      const result = original(bookKey, ...args);
  ...
  function renderLocker(scene) {
    const modal = scene?.__sv35LockerModal;
    if (!modal?.panel) return;

    const content = modal.panel.querySelector("[data-sv35-locker-content]");
    if (!content) return;
    content.replaceChildren();

    const grid = document.createElement("div");
    grid.className = "sv35-locker-grid";

    const bagSection = document.createElement("section");
    bagSection.className = "sv35-locker-section";

    const lockerSection = document.createElement("section");
    lockerSection.className = "sv35-locker-section";
    ...
    bagList.className = "sv35-locker-list";

    const lockerList = document.createElement("div");
    lockerList.className = "sv35-locker-list";
    ...
    if (!bagItems.length) {
      ...
      empty.className = "sv35-locker-empty";
  ...
  const DOM_MODAL_REFS_V35 = Object.freeze([
    "ticketModal",
    "itemsModal",
    "lootModal",
    "lionChoiceModal",
  ...
  function clearDetachedModalRefs(scene) {
    ...
    DOM_MODAL_REFS_V35.forEach((key) => {
  ...
  function hasHardWorldLock(scene) {
    ...
    return Boolean(
      scene.__sv35LockerModal ||
      scene.__sv35ZofingiaOpen ||
      scene.ticketModal ||
      scene.itemsModal ||
      scene.lootModal ||
  ...
  function closeLocker(scene, silent = false) {
    ...
    document
      .querySelectorAll(
        '#phaser-game [data-simon-ui="locker-v35"], ' +
  ...
  function openLocker(scene) {
    if (!scene || scene.__sv35LockerModal) return;
    ...
    content.dataset.sv35LockerContent = "true";
  ...
  function ensureLocker(scene) {
    ...
    const old = scene.__sv35LockerObjects;
    ...
    scene.__sv35LockerObjects = { body, label, zone };
  }
  ...
  function patchLockerScene(scene) {
    ...
    // Crucially, v35 does NOT wrap openItemsModal at all.
    if (
      scene.__sv35LockerModal &&
      !scene.__sv35LockerModal.overlay?.isConnected
    ) {
      scene.__sv35LockerModal = null;
      recoverWorldInput(scene);
    }
    ...
    if (
      ...
      !scene.create.__sv35LockerCreate
    ) {
      const original = scene.create.bind(scene);

      const wrapped = function createV35(...args) {
        const result = original(...args);
        this.time?.delayedCall?.(0, () => ensureLocker(this));
        return result;
      };

      wrapped.__sv35LockerCreate = true;
  ...
  function cleanupIndianRoom() {
    ...
    if (room?._sv35Timer) {
      window.clearInterval(room._sv35Timer);
    }
  ...
  function buildIndianRoom(scene) {
    ...
    if (document.querySelector('#phaser-game [data-simon-ui="inder-v35-room"]')) {
      return;
    }
    ...
    room.dataset.simonUi = "inder-v35-room";

    const bg = document.createElement("img");
    bg.className = "sv35-inder-room-bg";
    bg.src = "inder-shop-v35.png?v=35";
    bg.alt = "";
    bg.draggable = false;

    const seller = document.createElement("div");
    seller.className = "sv35-inder-seller";
    seller.setAttribute("role", "button");
    seller.setAttribute("aria-label", "Mit dem Verkäufer handeln");
    seller.tabIndex = 0;

    const bubble = document.createElement("div");
    bubble.className = "sv35-inder-bubble";
    ...
    room._sv35Timer = window.setInterval(() => {
  ...
  function fixGirlFacing(hive) {
    ...
    if (!woman.__sv35FlipInterceptor) {
      const rawSetFlipX = woman.setFlipX.bind(woman);
      woman.__sv35RawSetFlipX = rawSetFlipX;

      woman.setFlipX = function setFlipXInvertedV35(value) {
        return rawSetFlipX(!Boolean(value));
      };

      woman.__sv35FlipInterceptor = true;
    }
    ...
    woman.__sv35RawSetFlipX?.(desiredFlip);
  ...
  function closeEnriqueMenu(scene) {
    if (!scene?.__sv35EnriqueModal) return;
    try {
      scene.destroyDOMModal?.(scene.__sv35EnriqueModal);
    } catch {
      scene.__sv35EnriqueModal.overlay?.remove?.();
    }
    scene.__sv35EnriqueModal = null;
  }

  function openEnriqueMenu(scene) {
    if (!scene?.__sv35ZofingiaOpen || scene.__sv35EnriqueModal) return;
    ...
    scene.__sv35EnriqueModal = modal;
    ...
    answer.dataset.sv35EnriqueAnswer = "true";
  ...
  function cleanupZofingia(scene, recover = true) {
    ...
    const controls = document.querySelector(
      '#phaser-game [data-simon-ui="zofingia-v35-controls"]'
    );
    controls?.remove?.();

    if (scene.__sv35ZofingiaOverlay) {
      try {
        (scene.__sv35ZofingiaGuests || []).forEach((guest) => {
          scene.tweens?.killTweensOf?.(guest);
        });
        scene.__sv35ZofingiaOverlay.destroy(true);
      } catch {}
    }

    scene.__sv35ZofingiaOverlay = null;
    scene.__sv35ZofingiaGuests = [];
    scene.__sv35ZofingiaOpen = false;
  ...
  function openZofingia(scene) {
    ...
    bg.fillStyle(0x17120f, 1);
    bg.fillRect(0, 0, 820, 390);

    // dark Zurich club interior: walnut panelling, cream plaster, red carpet
    ...
    // tall windows with hints of the lake at night
    [58, 620].forEach((x) => {
    ...
    // red-white connection banners, without copying a real logo
    [255, 502].forEach((x) => {
    ...
    const enrique = createEnrique(scene, 410, 315);
    ...
    // Simon is represented inside the room so the entry reads as a location,
    // even though movement is intentionally not added yet.
    let simon = null;
    try {
      simon = scene.add.sprite(208, 317, "simon", 0)
        .setScale(0.28)
        .setDepth(736);
    ...
    scene.__sv35ZofingiaOverlay = overlay;
    scene.__sv35ZofingiaGuests = guests;
    scene.__sv35Enrique = enrique;

    const root = document.getElementById("phaser-game");
    if (root) {
      const controls = document.createElement("div");
      controls.dataset.simonUi = "zofingia-v35-controls";
  ...
  function ensureZurichPromenade(scene) {
    ...
    const old = scene.__sv35Promenade;
    ...
    sky.fillStyle(0x7697a8, 1);
    sky.fillRect(2440, 76, 560, 207);
    ...
    zone.on("pointerdown", (pointer) => {
      ...
      if (scene.uiLocked || scene.playerDying || scene.__sv35ZofingiaOpen) {
        return;
      }
    ...
    scene.__sv35Promenade = { background, front, zone };

    if (!scene.__sv35PromenadeShutdownHook) {
      scene.__sv35PromenadeShutdownHook = true;
      scene.events?.on?.("shutdown", () => {
        cleanupZofingia(scene, false);
        scene.__sv35Promenade = null;
      });
    }
  }
  ...
  function frame() {
    ...
    if (game) {
      ...
      if (station) {
        ...
        if (station.__sv35ZofingiaOpen && !station.uiLocked) {
  ...
  window.SimonUIV35 = Object.freeze({
    version: 35,
    locker: LOCKER_STATE,
    status() {
      return {
  ...
  console.info(
    "Simon UI v35 geladen: Locker/input stability + Woman facing + Zürichpromenade/Zofingia."
  );
})();