(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V55__) return;
  ...
  const POLYBAHN_OFFSET_FROM_LOCKER = 82;
  ...
  function showLocalNotice(scene, title, detail = "", duration = 2400) {
    ...
    node.dataset.simonUi = "eth-notice-v55";
  ...
  function canUsePolybahn(scene) {
    return Boolean(
      ...
      !scene.__polybahnTravelActiveV55
    );
  }
  ...
  function destroyBahnhofPolybahn(scene) {
    const entry = scene?.__ethCampusEntryV55;
    if (!entry) return;
    ...
    scene.__ethCampusEntryV55 = null;
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV55) return;
    ...
    g.fillRoundedRect(x - 34, 208, 68, bottom - 208, 5);

    g.fillStyle(0xb63132, 1);
    g.fillRoundedRect(x - 39, 179, 78, 34, 5);
    g.lineStyle(2, 0xf1dfc5, 1);
    g.strokeRoundedRect(x - 39, 179, 78, 34, 5);

    g.fillStyle(0x29454e, 1);
    g.fillRoundedRect(x - 24, 226, 48, bottom - 226, 4);
  ...
  function prepareBahnhofForPolybahn(station) {
    ...
    station.__polybahnReturnYV55 = station.player?.y || 235;
  ...
  function restoreBahnhofAfterPolybahn(station) {
    ...
    station.__ethCampusEnteringV55 = false;
    ...
    station.player?.setPosition?.(
      ...
      Number(station.__polybahnReturnYV55) || 235
    );
  ...
  function startPolybahnFromBahnhof(station) {
    ...
    if (
      ...
      console.error("ETH v55: Polybahn-Szenen fehlen.");
      return false;
    }

    prepareBahnhofForPolybahn(station);

    station.__ethCampusEnteringV55 = true;
    ...
    station.time?.delayedCall?.(180, () => {
      ...
      try {
        ...
        console.error("ETH v55: Polybahn-Start fehlgeschlagen:", error);
  ...
  class PolybahnTransitScene extends Phaser.Scene {
    constructor() {
      ...
      this.overworld = null;
      this.direction = "up";
      this.__finishedV55 = false;
      this.__startedAtV55 = 0;
    }
    ...
    create() {
      ...
      this.__startedAtV55 = performance.now();
      ...
      this.time.delayedCall(5200, () => {
        if (
          ...
          console.warn("ETH v55: Polybahn-Transit per Failsafe beendet.");
          this.finishTransit();
        }
      });
    ...
    createTransitVisuals() {
      ...
      houses.add(hg);
      this.__transitHousesV55 = houses;
    ...
    startTransit() {
      ...
      this.tweens.add({
        targets: this.__transitHousesV55,
        y: up ? 74 : -74,
        x: up ? -22 : 22,
        duration: 2850,
        ease: "Sine.easeInOut"
      ...
      this.tweens.add({
        targets: this.__transitCabinV55,
        x: target.x,
        y: target.y,
        duration: 2850,
        ease: "Sine.easeInOut",
    ...
    finishTransit() {
      ...
      this.time.delayedCall(155, () => {
        try {
          ...
          console.error("ETH v55: Polybahn-Ende fehlgeschlagen:", error);
    ...
    rideDown() {
      ...
      this.time.delayedCall(170, () => {
        try {
          ...
          console.error("ETH v55: Rückfahrt konnte nicht starten:", error);
    ...
    enterETH() {
      ...
      this.time.delayedCall(190, () => {
        try {
          ...
          console.error("ETH v55: ETH-Innenraumstart fehlgeschlagen:", error);
...
// Einstein dialogue / quiz helpers
  ...
  function createDialogueOverlay(scene, onAdvance) {
    ...
    overlay.dataset.simonUi = "eth-dialogue-v55";
  ...
  function openQuizModal(scene, question) {
    ...
    const overlay = document.createElement("div");
    overlay.dataset.simonUi = "eth-quiz-v55";
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV55(...args) {
  ...
  function ensureRegistered(game) {
    ...
    scenes.forEach(([key, SceneClass]) => {
      ...
      try {
        ...
        console.error(
          `ETH v55: ${key} konnte nicht registriert werden:`,
          error
        );
      }
    });
  ...
  function recoverPolybahn(game) {
    ...
    const eth = getScene(game, ETH_KEY);

    const transitActive = Boolean(transit?.sys?.isActive?.());
    const terraceActive = Boolean(terrace?.sys?.isActive?.());
    const ethActive = Boolean(eth?.sys?.isActive?.());

    // If a destination scene is active, Bahnhofstrasse must not also advance
    // underneath it.
    if (
    ...
    // Hard recovery for an interrupted return transition: Bahnhof is still
    // paused but none of the three Polybahn/ETH scenes exists anymore.
    if (
      station &&
      station.__polybahnTravelActiveV55 &&
      !transitActive &&
    ...
    // ETH must never run while its terrace parent is also updating.
    if (
      ethActive &&
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV55(options = {}) {
  ...
  console.info(
    "ETH Campus v55: Polybahn direkt am Locker, robuste Transit-Szene, Polyterrasse outdoor, Einstein mittig."
  );
})();
