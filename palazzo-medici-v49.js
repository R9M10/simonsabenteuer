(() => {
  ...
  if (window.__SIMON_PALAZZO_MEDICI_V49__) return;
  window.__SIMON_PALAZZO_MEDICI_V49__ = true;
  ...
  function createPalazzoFacade(scene) {
    ...
    const doorZone = scene.add.zone(doorX, bottom - 46, 150, 126)
      .setDepth(275)
    ...
    doorZone.on("pointerdown", (pointer) => {
      if (
        scene.canUseWorldInteraction &&
        !scene.canUseWorldInteraction(pointer, { requireGround: false })
      ) {
        return;
      }
  ...
  function enterPalazzo(venice) {
    if (
      !venice?.sys?.isActive?.() ||
      venice.__palazzoMediciEnteringV49 ||
    ...
    const game = getGame() || venice.game || null;
    if (!game?.scene) {
      console.error("Palazzo Medici: Phaser SceneManager nicht gefunden.");
      return false;
    }
    ...
    if (!game.scene.keys?.PalazzoMediciScene) {
      console.error("Palazzo Medici: Innenraum konnte nicht registriert werden.");
      return false;
    }

    venice.__palazzoMediciEnteringV49 = true;
    venice.player.setVelocity?.(0, 0);
    venice.blockWorldInteractions?.(650);
    venice.setUILocked?.(true);

    const finishTransition = () => {
      try {
        // Use the Game SceneManager directly. This remains valid even while
        // Venice itself is being paused and avoids the old self-pause/launch race.
        game.scene.pause("VeniceScene");
        game.scene.start("PalazzoMediciScene", {
          overworld: venice
        });

        venice.__palazzoMediciEnteringV49 = false;
        return true;
      } catch (error) {
        console.error("Palazzo Medici: Eintritt fehlgeschlagen:", error);

        venice.__palazzoMediciEnteringV49 = false;
        venice.cameras?.main?.resetFX?.();
        venice.setUILocked?.(false);
        venice.setControlsVisible?.(true);
        return false;
      }
    };

    if (venice.cameras?.main?.fadeOut) {
      venice.cameras.main.fadeOut(220, 0, 0, 0);

      window.setTimeout(() => {
        if (!venice.sys?.isActive?.()) {
          // Venice may already have been paused by another wrapper; still try
          // the SceneManager transition rather than silently abandoning it.
          finishTransition();
          return;
        }
  ...
  function patchVenice(scene) {
    if (!scene || scene.__palazzoMediciV49Installed) return;
    scene.__palazzoMediciV49Installed = true;

    if (typeof scene.create === "function") {
      const originalCreate = scene.create.bind(scene);

      const wrappedCreate = function createWithPalazzoMediciV49(...args) {
      ...
      wrappedCreate.__palazzoMediciV49 = true;
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGamePalazzoMediciV49(options = {}) {
  ...
  window.PalazzoMediciV49 = Object.freeze({
    install,
    enterPalazzo
  });
})();
