(() => {
  ...
  if (window.__SIMON_ETH_CAMPUS_V57__) return;
  ...
  function refreshCampusEntryAvailability(scene) {
    const entry = scene?.__ethCampusEntryV57;
    if (!entry) return;
  ...
  function destroyBahnhofPolybahn(scene) {
    ...
    scene.__ethCampusEntryV57 = null;
  }

  function createCampusEntry(scene) {
    if (!scene || scene.__ethCampusEntryV57) return;

    const x = getPolybahnBahnhofX();
    const bottom = GROUND_TOP;

    const facade = scene.add.container(0, 0).setDepth(8);
    const g = scene.add.graphics();

    // Wider, unmistakable Polybahn station. Kept fully left of the tram.
    g.fillStyle(0x9a9185, 1);
    g.fillRoundedRect(x - 65, 176, 130, bottom - 176, 7);

    // Stone top / cornice.
    g.fillStyle(0xc8bda9, 1);
    g.fillRect(x - 72, 169, 144, 13);

    // Red Polybahn band.
    g.fillStyle(0xb63132, 1);
    g.fillRoundedRect(x - 68, 187, 136, 34, 5);

    g.lineStyle(3, 0xf1dfc5, 1);
    g.strokeRoundedRect(x - 68, 187, 136, 34, 5);

    // Arched station opening.
    g.fillStyle(0x2b444d, 1);
    g.fillCircle(x, 258, 46);
    g.fillRect(x - 46, 258, 92, bottom - 258);

    g.fillStyle(0x1d333b, 1);
    g.fillCircle(x, 260, 35);
    g.fillRect(x - 35, 260, 70, bottom - 260);

    g.lineStyle(4, 0xdfd4bf, 1);
    g.strokeCircle(x, 258, 46);

    // Central glass/door split.
    g.lineStyle(2, 0x9fb7bd, 1);
    g.lineBetween(x, 267, x, bottom - 4);

    // Tiny red carriage pictogram above the door.
    g.fillStyle(0xc43a3a, 1);
    g.fillRoundedRect(x - 18, 226, 36, 21, 4);
    g.fillStyle(0xdce6e6, 1);
    g.fillRect(x - 12, 231, 9, 8);
    g.fillRect(x + 3, 231, 9, 8);
  ...
  function startPolybahnFromBahnhof(station) {
    ...
    if (
      ...
      console.error("ETH v57: Polybahn-Szenen fehlen.");
      return false;
    }
    ...
    station.time?.delayedCall?.(180, () => {
      ...
      try {
        ...
        console.error(
          "ETH v57: Polybahn-Start fehlgeschlagen:",
          error
        );
    ...
    create() {
      ...
      this.time.delayedCall(5200, () => {
        if (
          ...
          console.warn("ETH v57: Polybahn-Transit per Failsafe beendet.");
          this.finishTransit();
        }
      });
    ...
    finishTransit() {
      ...
      this.time.delayedCall(155, () => {
        try {
          ...
          console.error(
            "ETH v57: Polybahn-Ende fehlgeschlagen:",
            error
          );
    ...
    rideDown() {
      ...
      this.time.delayedCall(170, () => {
        try {
          ...
          console.error("ETH v57: Rückfahrt konnte nicht starten:", error);
    ...
    enterETH() {
      ...
      this.time.delayedCall(190, () => {
        try {
          ...
          console.error("ETH v57: ETH-Innenraumstart fehlgeschlagen:", error);
...
// Einstein dialogue / quiz helpers
    ...
    leaveETH() {
      ...
      this.time.delayedCall(200, () => {
        try {
          ...
          console.error("ETH v57: Rückkehr auf Polyterrasse fehlgeschlagen:", error);
        } finally {
  ...
  function patchBahnhofPrototype() {
    ...
    const wrapped = function createWithEthCampusV57(...args) {
  ...
  function ensureRegistered(game) {
    ...
    scenes.forEach(([key, SceneClass]) => {
      ...
      try {
        ...
        console.error(
          `ETH v57: ${key} konnte nicht registriert werden:`,
          error
        );
      }
    });
  ...
  if (typeof previousStart === "function") {
    window.startSimonGame = function startSimonGameEthV57(options = {}) {
  ...
  console.info(
    "ETH Campus v57: Polybahn links von der Tram, robuste Transit-Szene, Polyterrasse outdoor, Einstein mittig."
  );
})();
