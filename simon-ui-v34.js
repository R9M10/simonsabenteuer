(() => {
  ...
  if (window.__SIMON_UI_V34__) return;
  window.__SIMON_UI_V34__ = true;
  ...
  const LOCKER_STATE = window.__SIMON_LOCKER_V34__ || {
  ...
  window.__SIMON_LOCKER_V34__ = LOCKER_STATE;
  ...
  function ensureStyles() {
    if (document.getElementById("simon-ui-v34-style")) return;

    const style = document.createElement("style");
    style.id = "simon-ui-v34-style";
    style.textContent = `
      #phaser-game [data-simon-ui="inder-v34-room"] {
        position: absolute;
        inset: 0;
        z-index: 99980;
        overflow: hidden;
      ...
      #phaser-game .sv34-inder-room-bg {
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
      ...
      #phaser-game .sv34-inder-seller {
        ...
        height: 170px;
        z-index: 3;
        background-image: url("inder-sprites-v34.png?v=33");
      ...
      #phaser-game .sv34-locker-section {
        min-width: 0;
      ...
      @media (max-width: 620px) {
        ...
        #phaser-game .sv34-locker-row button {
          grid-column: 1 / -1;
        }
      }
    `;
  ...
  function patchPurchases(scene) {
    ...
    if (
      ...
      !scene.purchaseStoreItem.__sv34AutoHotbar
    ) {
      const original = scene.purchaseStoreItem.bind(scene);

      const wrapped = function purchaseStoreItemV34(itemKey, ...args) {
      ...
      wrapped.__sv34AutoHotbar = true;
      scene.purchaseStoreItem = wrapped;
    }

    if (
      typeof scene.purchaseBook === "function" &&
      !scene.purchaseBook.__sv34AutoHotbar
    ) {
      const original = scene.purchaseBook.bind(scene);

      const wrapped = function purchaseBookV34(bookKey, ...args) {
  ...
  function patchQuoteBanner(scene) {
    if (
      ...
      scene.showRandomBookQuote.__sv34QuoteSize
    ) {
      return;
    }
    ...
    const wrapped = function showRandomBookQuoteV34(bookKey, ...args) {
      const result = original(bookKey, ...args);
  ...
  function renderLocker(scene) {
    const modal = scene?.__sv34LockerModal;
    if (!modal?.panel) return;

    const content = modal.panel.querySelector("[data-sv34-locker-content]");
    if (!content) return;
    content.replaceChildren();

    const grid = document.createElement("div");
    grid.className = "sv34-locker-grid";

    const bagSection = document.createElement("section");
    bagSection.className = "sv34-locker-section";

    const lockerSection = document.createElement("section");
    lockerSection.className = "sv34-locker-section";
    ...
    bagList.className = "sv34-locker-list";

    const lockerList = document.createElement("div");
    lockerList.className = "sv34-locker-list";
    ...
    if (!bagItems.length) {
      ...
      empty.className = "sv34-locker-empty";
  ...
  function openLocker(scene) {
    if (!scene || scene.__sv34LockerModal) return;
    ...
    content.dataset.sv34LockerContent = "true";
  ...
  function ensureLocker(scene) {
    ...
    const old = scene.__sv34LockerObjects;
    ...
    scene.__sv34LockerObjects = { body, label, zone };
  }
  ...
  function patchLockerScene(scene) {
    ...
    // Crucially, v34 does NOT wrap openItemsModal at all.
    if (
      scene.__sv34LockerModal &&
      !scene.__sv34LockerModal.overlay?.isConnected
    ) {
      scene.__sv34LockerModal = null;
      scene.refreshUILock?.();
    }
    ...
    if (
      ...
      !scene.refreshUILock.__sv34LockerAware
    ) {
      const original = scene.refreshUILock.bind(scene);

      const wrapped = function refreshUILockV34(...args) {
        const result = original(...args);
        if (this.__sv34LockerModal) {
          this.setUILocked?.(true);
        }
        return result;
      };

      wrapped.__sv34LockerAware = true;
    ...
    if (
      ...
      !scene.create.__sv34LockerCreate
    ) {
      const original = scene.create.bind(scene);

      const wrapped = function createV34(...args) {
        const result = original(...args);
        this.time?.delayedCall?.(0, () => ensureLocker(this));
        return result;
      };

      wrapped.__sv34LockerCreate = true;
  ...
  function cleanupIndianRoom() {
    ...
    if (room?._sv34Timer) {
      window.clearInterval(room._sv34Timer);
    }
  ...
  function buildIndianRoom(scene) {
    ...
    if (document.querySelector('#phaser-game [data-simon-ui="inder-v34-room"]')) {
      return;
    }
    ...
    room.dataset.simonUi = "inder-v34-room";

    const bg = document.createElement("img");
    bg.className = "sv34-inder-room-bg";
    bg.src = "inder-shop-v34.png?v=33";
    bg.alt = "";
    bg.draggable = false;

    const seller = document.createElement("div");
    seller.className = "sv34-inder-seller";
    seller.setAttribute("role", "button");
    seller.setAttribute("aria-label", "Mit dem Verkäufer handeln");
    seller.tabIndex = 0;

    const bubble = document.createElement("div");
    bubble.className = "sv34-inder-bubble";
    ...
    room._sv34Timer = window.setInterval(() => {
  ...
  function fixGirlFacing(hive) {
    ...
    // Verified in-game orientation:
    // unflipped woman-v14 faces LEFT; flipped faces RIGHT.
    // So she flips only when Simon stands to her RIGHT.
  ...
  window.SimonUIV34 = Object.freeze({
    version: 33,
    locker: LOCKER_STATE,
    status() {
      return {
  ...
  console.info(
    "Simon UI v34 geladen: Inder assets + Auto-Hotbar + Locker + quote/facing fixes."
  );
})();