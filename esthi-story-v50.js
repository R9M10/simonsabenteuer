(() => {
  ...
  if (window.__SIMON_ESTHI_STORY_V50__) return;
  window.__SIMON_ESTHI_STORY_V50__ = true;
  ...
  const WORLD = Object.freeze({
    ...
    // Original HIVE starts at x=1575. Move the whole display block,
    // including its bouncer and input objects, closer to Milchbuck station.
    hiveOriginalX: 1575,
    hiveTargetX: 1030,

    parkLeft: 1325,
    parkRight: 1990,
    parkTriggerLeft: 1375,
    parkTriggerRight: 1940,
  ...
  function showUnlock(scene) {
    ...
    notice.dataset.simonUi = "esthi-memory-v50";
  ...
  function patchHiveLocation() {
    ...
    if (
      ...
      proto.createHiveClub.__esthiHiveLocationV50
    ) {
      return;
    }
    ...
    const wrapped = function createHiveClubV50(...args) {
      if (!isMilchbuck(this)) {
      ...
      this.__esthiHiveShiftV50 = HIVE_SHIFT;
      return result;
    };

    wrapped.__esthiHiveLocationV50 = true;
  ...
  function showEsthiPermanently(scene, groundY = null) {
    const esthi = scene.__esthiV50;
    if (!esthi) return;

    const y =
      Number.isFinite(groundY)
        ? groundY
        : (Number(scene.__esthiGroundYV50) || 294);
  ...
  function makeStoryOverlay(onClick) {
    ...
    overlay.dataset.simonUi = "esthi-story-v50";
  ...
  function startEsthiWalkBob(scene, esthi) {
    scene.__esthiWalkBobV50?.stop?.();
    scene.__esthiWalkBobV50?.remove?.();

    scene.__esthiWalkBobV50 = scene.tweens.add({
      targets: esthi,
      y: esthi.y - 2,
      angle: { from: -1, to: 1 },
      duration: 170,
  ...
  function walkPair(scene, simonX, esthiX, done) {
    ...
    const groundY = Number(scene.__esthiGroundYV50) || player.y;
  ...
  function approachSimon(scene, done) {
    const player = scene.player;
    const esthi = scene.__esthiV50;
    const groundY = scene.__esthiGroundYV50;
  ...
  function startEsthiStory(scene) {
    ...
    scene.__esthiGroundYV50 = groundY;
  ...
  function patchMilchbuckLifecycle() {
    ...
    if (
      ...
      !proto.create.__esthiCreateV50
    ) {
      const originalCreate = proto.create;

      const wrappedCreate = function createV50(...args) {
      ...
      wrappedCreate.__esthiCreateV50 = true;
      proto.create = wrappedCreate;
    }

    if (
      typeof proto.update === "function" &&
      !proto.update.__esthiUpdateV50
    ) {
      const originalUpdate = proto.update;

      const wrappedUpdate = function updateV50(time, delta) {
      ...
      wrappedUpdate.__esthiUpdateV50 = true;
  ...
  window.SimonEsthiV50 = Object.freeze({
    VERSION,
    state,
    WORLD,
  ...
  console.info(
    "Esthi Story v50 geladen: HIVE näher, Park + SPAR + einmaliges Esthi-Event."
  );
})();